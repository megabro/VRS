...
tempSurrounding = 293;      % temperature of air surrounding the
...
kTank = 14.4;               % fuel/oxidizer tanks conduction coefficient
                            % (W/(m*K))
kIns = 0.016;               % fuel/oxidizer tank insulations conduction
                            % coefficient (W/(m*K))
xTank = 0.003;              % fuel/oxidizer tank thickness (m)
xIns = 0.002;               % fuel/oxidizer tank insulation thickness (m)
hAir = 20.83;               % convection coefficient of air
                            % (W/(m^2*K))

temp3 = (hAir * tempSurrounding + temp1 * (kTank / xTank) * ...
         (kTank / xTank - 1)) / (hAir - (kTank * kIns) / ...
         (xTank * xIns));
